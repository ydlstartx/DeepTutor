...
def test_session_folder_api_roundtrip(monkeypatch, tmp_path: Path) -> None:
    ...
    app = FastAPI()
    app.include_router(sessions_module.router, prefix="/api/sessions")

    with TestClient(app) as client:
        created = client.post("/api/sessions/folders", json={"name": "Work"})
        assert created.status_code == 201
        folder = created.json()["folder"]

        moved = client.put(
            f"/api/sessions/{session['id']}/folder",
        ...
        assert client.get("/api/sessions/folders").json()["folders"][0]["session_count"] == 1

        duplicate = client.post("/api/sessions/folders", json={"name": " work "})
        assert duplicate.status_code == 409

        deleted = client.delete(f"/api/sessions/folders/{folder['id']}")
...
def test_moving_parent_chat_keeps_little_tutor_in_same_folder(monkeypatch, tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        response = client.put(
            f"/api/sessions/{parent['id']}/folder",
            json={"folder_id": folder["id"]},
        )
