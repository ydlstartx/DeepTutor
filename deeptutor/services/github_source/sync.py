...
async def sync_source(kb_name, source, *, base_dir=DEFAULT_BASE_DIR, client=None):
    # A GitHub sync is a local KB build: it downloads into raw/, updates the
    # index, removes stale documents, and rewrites metadata. Reject it before
    # even creating raw/ on a query-only deployment. IMA remains separate — it
    # registers a remote retrieval pointer and never enters this pipeline.
