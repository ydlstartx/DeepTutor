...
class LightRagPipeline:
    ...
    async def search(self, query: str, kb_name: str, **kwargs) -> Dict[str, Any]:
        kb_dir = resolve_kb_dir(self.kb_base_dir, kb_name)
        from .legacy_query import latest_legacy_root, query_view

        root_dir = storage.latest_published_root(kb_dir)
        legacy = root_dir is None
        if legacy:
            root_dir = latest_legacy_root(kb_dir)
        if root_dir is None:
            return {
                "query": query,
        ...
        try:
            ...
            async def job(io_bridge: OwnerLoopBridge):
                with query_view(root_dir) if legacy else nullcontext(root_dir) as query_root:
                    rag = engine.build_rag(
                        query_root,
                        io_bridge=io_bridge,
                        **({"legacy_source": root_dir} if legacy else {}),
                    )
                    failed = True
                    try:
                        await engine.initialize(rag)
