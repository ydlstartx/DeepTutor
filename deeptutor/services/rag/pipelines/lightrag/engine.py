    ...
    *,
    ...
    if enable_vlm:
        ...
        role_configs["vlm"] = RoleLLMConfig(
            func=build_vision_model_func(**vision_kwargs),
            metadata=metadata,
            max_async=int(load_lightrag_settings().get("multimodal_concurrency", 8)),
        )
    if role_configs:
        constructor["role_llm_configs"] = role_configs
    from .storage import read_vector_storage

    selected_vector_storage = read_vector_storage(legacy_source or working_dir)
    if selected_vector_storage not in {"nano", "faiss"}:
        raise LightRagContractError(f"Unsupported vector storage: {selected_vector_storage}")
    if selected_vector_storage == "faiss":
        from .faiss_compat import install_lean_loader

        install_lean_loader()
        constructor["vector_storage"] = "FaissVectorDBStorage"
    if legacy_source is not None:
        from .legacy_query import make_read_only

        constructor["workspace"] = workspace_for(legacy_source)
        vector_storage = read_vector_storage(legacy_source)
        if vector_storage not in {"nano", "faiss"}:
            raise LightRagContractError(f"Unsupported legacy vector storage: {vector_storage}")
        constructor["vector_storage"] = (
            "FaissVectorDBStorage" if vector_storage == "faiss" else "NanoVectorDBStorage"
        )
        constructor["enable_llm_cache"] = False
        constructor["enable_llm_cache_for_entity_extract"] = False
        rag = _controlled_class()(**constructor)
        make_read_only(rag)
        return rag
