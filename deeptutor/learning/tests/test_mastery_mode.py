...
def _pack(language: str) -> dict:
    ...
    return yaml.safe_load(prompt.read_text(encoding="utf-8"))


def test_study_never_offers_to_do_the_reviewing_itself(monkeypatch, tmp_path):
...
def test_naming_an_activity_is_documented_as_a_mode_request(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    for language in ("zh", "en"):
...
def test_the_outline_metaphor_is_gone_from_learner_facing_copy(monkeypatch, tmp_path):
