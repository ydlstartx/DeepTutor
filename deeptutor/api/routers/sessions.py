...
async def move_session_to_folder(session_id: str, payload: SessionFolderMoveRequest):
    ...
    if not moved:
        raise HTTPException(status_code=404, detail="Session not found")
    # Little Tutor threads are organizational children of their source chat.
    # Keep the dev folder enhancement consistent with main's course/archive
    # cascade so a child never disappears into a different history folder.
    candidates = await list_all_sessions_snapshot(store)
    for candidate in candidates:
        preferences = candidate.get("preferences") or {}
        if str(preferences.get("parent_session_id") or "") == session_id:
            await store.move_session_to_folder(candidate["session_id"], payload.folder_id)
