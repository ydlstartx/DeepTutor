...
export interface SessionSummary {
  ...
  /** Optional for sessions created before folder support was introduced. */
...
export async function listSessionFolders(options?: {
  ...
  return withClientCache<SessionFolder[]>(
    "session-folders",
    async () => {
      const response = await apiFetch(apiUrl("/api/sessions/folders"), {
        cache: "no-store",
      });
...
export async function renameSessionFolder(
  ...
  const response = await apiFetch(
    apiUrl(`/api/sessions/folders/${folderId}`),
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
...
export async function moveSessionToFolder(
  ...
  const response = await apiFetch(
    apiUrl(`/api/sessions/${sessionId}/folder`),
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
