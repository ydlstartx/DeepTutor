...
export async function getKnowledgeBasePolicy(options?: { force?: boolean }) {
  return withClientCache<KnowledgeBasePolicy>(
    ...
    async () => {
      const response = await apiFetch(apiUrl("/api/knowledge-bases/policy"), {
        cache: "no-store",
      });
...
export async function listKnowledgeImportFolders(
  ...
  const res = await apiFetch(
    apiUrl(`/api/knowledge-bases/import/folders${suffix ? `?${suffix}` : ""}`),
    { cache: "no-store" },
  );
  if (!res.ok) {
...
export async function probeKnowledgeImportFolder(
  ...
  const res = await apiFetch(apiUrl("/api/knowledge-bases/import/probe"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function importExistingKnowledgeBase(payload: {
  ...
  const res = await apiFetch(apiUrl("/api/knowledge-bases/import"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function renameKbFile(
  ...
  const res = await apiFetch(
    apiUrl(`/api/knowledge-bases/${encodeURIComponent(name)}/files/rename`),
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
