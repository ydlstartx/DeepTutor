    ...
    <div className="min-w-0 flex-1 overflow-y-auto bg-[var(--background)]">
      <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {onImport && <button type="button" onClick={onImport} className="ml-auto rounded-lg border border-[var(--border)] px-3 py-2 text-[12.5px]">{t("Import knowledge base")}</button>}
          <button
            type="button"
            onClick={onCreate}
          ...
          >
            <Plus size={14} />
            {t(queryOnly ? "Connect IMA knowledge base" : "New knowledge base")}
          </button>
        </div>

        {queryOnly && <p className="mt-4 text-sm text-[var(--muted-foreground)]">{t("Knowledge bases are built offline and published by an administrator. Existing knowledge bases remain available for retrieval.")}</p>}
        <div
          role="tablist"
        ...
        {activeSection === "knowledge-bases" ? (
          ...
          >
            ...
            <section>
              ...
              <div className="grid grid-cols-1 items-stretch gap-3 md:grid-cols-2">
                {!queryOnly && externalSources.map(renderExternalSource)}
              </div>
            </section>
          </div>
        )}
