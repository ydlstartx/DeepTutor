...
export function makeFileLinkRemarkPlugin(files: MessageAttachment[]) {
  ...
  for (const file of files) {
    const name = file.origin === "workspace" ? file.relative_path : file.filename;
    if (!name) continue;
    const variants = file.origin === "workspace" ? new Set([name, `./${name}`]) : new Set([name, name.replace(/[_-]+/g, " ")]);
  ...
  const imageNames = new Set(
    files
      ...
      .map((file) => file.origin === "workspace" ? file.relative_path : file.filename)
  ...
  const linkTargetFile = (
    ...
    return (
      lookupSurface(decode(url)) ?? lookupSurface(decode(baseName(url))) ?? lookupSurface(linkLabel(node))
    );
  };
